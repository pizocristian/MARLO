...
public class EditCenterProjectsInterceptor extends AbstractInterceptor implements Serializable {

  /**
   * 
   */
  private static final long serialVersionUID = 5712697573159769112L;
  ...
  @Inject
  public EditCenterProjectsInterceptor(ProjectManager projectManager, GlobalUnitManager crpManager,
    ...
    if (loggedCrp.isCenterType()) {
      if (project != null && project.isActive()) {
        ...
        if (projectPhase != null) {
          if (!projectPhase.getCrp().getId().equals(loggedCrp.getId())) {
            GlobalUnitProject globalUnitProject =
              globalUnitProjectManager.findByProjectAndGlobalUnitId(project.getId(), projectPhase.getCrp().getId());
            if (globalUnitProject != null) {
              if (project != null && project.isActive()) {
                ...
                String params[] = {crp.getAcronym(), project.getId() + "",
                  baseAction.getActionName().replaceAll(crp.getAcronym() + "/", "")};
                if (baseAction.canAccessSuperAdmin() || baseAction.canEditCrpAdmin()) {
                  ...
                  LiaisonUser lUser =
                    liaisonUserManager.getLiaisonUserByUserId(user.getId(), projectPhase.getCrp().getId());
                    ...
                    if (projectPartner != null) {
                      ...
                      if (institutionCp != null) {
                        if (institutionCp.getId().equals(institutionProject.getId())) {
                          ...
                          if (baseAction
                            .hasPermission(baseAction.generatePermission(Permission.PROJECT__SWITCH, params))) {
                            canSwitchProject = true;
                          }
                        }
                      } else {
                ...
                if (baseAction.isReportingActive()
                  && actionName.contains(ProjectSectionStatusEnum.BUDGET.getStatus())) {
