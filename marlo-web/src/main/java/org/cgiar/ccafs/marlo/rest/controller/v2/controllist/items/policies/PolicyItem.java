                ...
                if (fieldErrors.isEmpty()) {
                    ...
                    if (projectPolicy != null) {
                        ...
                        try {
                            ...
                            RestApiAuditlog restApiAuditLog = new RestApiAuditlog("createPolicy", "Created " + projectPolicy.getId(), new Date(), projectPolicy.getId(), "class org.cgiar.ccafs.marlo.data.model.ProjectPolicy",
    ...
    public Long putPolicyById(Long id, NewProjectPolicyDTO newPolicyDTO, String entityAcronym, User user) {
        ...
        if (!fieldErrors.isEmpty()) {
            ...
            try {
                ObjectMapper mapper = new ObjectMapper();
                String originalJson = mapper.writerWithDefaultPrettyPrinter().writeValueAsString(newPolicyDTO);
                RestApiAuditlog restApiAuditLog = new RestApiAuditlog("updatePolicy", "Updated " + projectPolicy.getId(), new Date(), projectPolicy.getId(), "class org.cgiar.ccafs.marlo.data.model.ProjectPolicy",
                        originalJson, user.getId(), null, "", phase.getId());
                restApiAuditlogManager.logApiCall(restApiAuditLog);
            } catch (Throwable ex) {
                Logger.getLogger(PolicyItem.class.getName()).log(Level.SEVERE, null, ex);
