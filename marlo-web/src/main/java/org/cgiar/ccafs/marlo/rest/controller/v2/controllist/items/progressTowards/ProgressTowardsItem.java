...
public class ProgressTowardsItem<T> {
            ...
            if (fieldErrors.isEmpty()) {
                ...
                if (reportSynthesisSrfProgressTargetDB != null) {
                    ...
                    try {
                        ...
                        RestApiAuditlog restApiAuditLog = new RestApiAuditlog("createProgressTowards", "Created CGIAR Entity Acronym " + CGIARentityAcronym + " ID " + srfProgressTargetId, new Date(), srfProgressTargetId, "class org.cgiar.ccafs.marlo.data.model.ReportSynthesisSrfProgressTarget",
     ...
     * @param year
     * @param phase
     * @return a SrfProgressTowardsTargetDTO with the
     * reportSynthesisSrfProgressTarget Item
     */
    ...
    public Long putProgressTowardsById(Long idProgressTowards,
        ...
        try {
            ObjectMapper mapper = new ObjectMapper();
            String originalJson = mapper.writerWithDefaultPrettyPrinter().writeValueAsString(newSrfProgressTowardsTargetDTO);
            RestApiAuditlog restApiAuditLog = new RestApiAuditlog("updateProgressTowards", "Updated " + reportSynthesisSrfProgressTarget.getId(), new Date(), reportSynthesisSrfProgressTarget.getId(), "class org.cgiar.ccafs.marlo.data.model.ReportSynthesisSrfProgressTarget",
                    originalJson, user.getId(), null, "", phase.getId());
            restApiAuditlogManager.logApiCall(restApiAuditLog);
        } catch (JsonProcessingException ex) {
            Logger.getLogger(ProgressTowardsItem.class.getName()).log(Level.SEVERE, null, ex);
